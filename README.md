...
Страница `my/` собирается из секций — независимо редактируемых блоков (имя, фото, email, телефон, адрес, пароль, связанные аккаунты, удаление аккаунта). Какие секции существуют и какие из них доступны, решает приложение: видимость складывается из трёх независимых источников (`personal_fields` сайта, `login_methods` домена, реально привязанные аккаунты контакта), и `my.profile.html` их не проверяет — он обходит готовый массив `$profile_groups` и выводит `html` каждой секции.
...
| `$mode` | `view` или `edit` |
| `$index` | Номер значения для многозначных секций (адреса), иначе `null` |
